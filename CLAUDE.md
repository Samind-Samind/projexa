...
ก่อนรันครั้งแรก ให้คัดลอก `app/js/firebase-config.example.js` เป็น `app/js/firebase-config.js` แล้วใส่ค่าจริงจาก Firebase Console ของโปรเจกต์ (ไฟล์ `firebase-config.js` ถูก `.gitignore` ไว้แล้ว จะไม่ถูก commit/push — ดูหัวข้อ "ข้อห้ามเรื่อง credential" ด้านล่าง) จากนั้นรันดูได้ด้วย `cd app && npx serve -l 3000 .` แล้วเปิด `http://localhost:3000/login.html` (ES module ต้องเสิร์ฟผ่าน http ไม่ใช่เปิดไฟล์ตรงๆ — ไม่มีไฟล์ `index.html` ที่ root ของ `app/` แล้ว เพราะลบทิ้งไปตามคำขอ user คำแนะนำ first-run เดิมที่เคยอยู่ในนั้นย้ายมารวมไว้ในหัวข้อนี้แทน) ไม่มี lint/test suite สำหรับ `app/` ในตอนนี้

ระบบมีล็อกอินด้วย Firebase Authentication (Email/Password) แล้ว (`app/login.html`, `app/signup.html`, `app/js/auth.js`) — ต้องเปิดใช้งาน Email/Password provider ใน Firebase Console → Authentication → Sign-in method เองก่อน แล้วสมัครสมาชิกผ่าน `signup.html` (สร้างทั้ง Firebase Auth account และเอกสารใน collection `users` ผูกกันด้วย email ให้อัตโนมัติ — ดู [screen-tracking-nosql-module-tech.md](docs/02-design/02-technical/screen-tracking-nosql-module-tech.md) และ auth flow ที่เกี่ยวข้อง) ทุกหน้าจอยกเว้น `login.html`/`signup.html` ต้องล็อกอินก่อนถึงจะใช้งานได้ (ตรวจสอบผ่าน `app/js/common.js` ซึ่งเป็น ES module ที่ redirect ไป `login.html` ถ้ายังไม่ได้ล็อกอิน) การใส่ข้อมูลตัวอย่างขึ้น Firestore ครั้งแรก (จาก `app/js/data.js`) ให้เปิด `app/seed.html` แล้วกดปุ่มบนหน้านั้น (`app/js/seed.js` รันครั้งเดียวตอนตั้งค่าโปรเจกต์ ไม่ได้ใช้ในหน้าจอปกติ — ต้องสมัครสมาชิกผู้ใช้ตัวอย่างให้ครบผ่าน `signup.html` ก่อน เพราะ `seed.js` จะจับคู่ชื่อผู้รับผิดชอบในข้อมูลตัวอย่างกับ `user_id` จริงจากบัญชีที่สมัครไว้) — `app/firestore.rules` บังคับ `allow read, write: if request.auth != null` (ต้องล็อกอินก่อนเท่านั้น ยังไม่แยกสิทธิ์ตามบทบาท SA/ผู้รับผิดชอบ/PM เพราะ role เป็นแค่แนวคิดในสโคปนี้) อย่าแก้ให้รัดกุมขึ้น (เช่นแยกสิทธิ์ตาม role จริง) เองโดยไม่ถาม user ก่อน
...
02-design/        → 01-prototypes (ต้นแบบ UI/UX) → 02-technical (สถาปัตยกรรม, DB schema, การออกแบบ API — แยกเป็น tech-stack.md ระดับระบบ และ detailed-design/ ระดับหน้าจอ)
...
- [.claude/skills/detailed-design/SKILL.md](.claude/skills/detailed-design/SKILL.md) สร้าง/ปรับปรุงเอกสาร **Detailed Design ระดับ Conceptual Design** ต่อหน้าจอ/ฟีเจอร์ที่ `docs/02-design/02-technical/detailed-design/scr-XXX-{slug}.md` (ระบุ actor/layer/component ตาม §3, entity ตาม §4.1, business rule, บังคับมี **Sequence Diagram** แบบ mermaid อย่างน้อย 1 diagram, data flow, error/exception flow, traceability) โดยถาม user ทุกจุดที่ไม่ชัดเจนด้วย `AskUserQuestion` พร้อมตัวเลือกอย่างน้อย 3 แนวทางเสมอ (สโคป, ระดับความละเอียดของ diagram, โหมดแก้ไฟล์เดิม) ทำงานแบบ interactive ใน main loop เสมอ แล้วมอบหมายให้ sub-agent [.claude/agents/detailed-design-writer.md](.claude/agents/detailed-design-writer.md) (one-shot, `Read/Write/Edit/Glob/Grep`) เขียนไฟล์จริงเมื่อเนื้อหาถูก finalize และยืนยันแล้วเท่านั้น — ใช้ skill นี้เมื่อ user ขอสร้าง/ปรับปรุง detailed design, conceptual design, sequence diagram/sequence flow ของหน้าจอหรือฟีเจอร์ใดในระบบ
- SCR ที่มี AI Component เกี่ยวข้องตามตาราง §7.1 (เช่น SCR-004/005/008/009/010/011/019/021–024) ต้องออกแบบ sequence diagram ให้สะท้อนกติกา §7.2 เสมอ (AI คืนค่า JSON เท่านั้น, มี source reference/confidence, ต้องมี step ให้คนยืนยันก่อนบันทึกจริง — ห้ามให้ AI บันทึกตรงลง Data Layer) และถ้าเกี่ยวกับการสร้างเอกสาร (§8) ต้องแยก step "AI ร่างเนื้อความบรรยาย" ออกจาก step "ระบบ/Document Generator ดึงตัวเลขและ render Template" ให้ชัดเจน — เอกสาร Detailed Design ไม่ใช่มุมมองสรุปแบบ feature-list/user-journey ห้ามเขียนทับหรือลบเนื้อหาเดิมโดยไม่ยืนยันกับ user ก่อน ถ้าล้าสมัยจริงให้ย้ายไป `00-archived/` แทน
