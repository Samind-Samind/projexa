...
- **วันที่อัปเดตล่าสุด:** 2026-08-24 — รอบแก้ไขล่าสุด (ปรับกลุ่มสี Accent ให้ตรงกับสีจริงของโลโก้ Projexa ต่อจากรอบก่อนหน้าที่ปรับพื้นหลัง/การ์ด/shadow ตาม mood board — ไม่ใช่การสร้างไฟล์ครั้งแรก)
...
โทนที่ยึดตลอดทั้งระบบ (อัปเดตรอบนี้): **Warm Minimalist + Gold & Umber + Soft-lift Card** — พื้นหลังครีมอุ่น, การ์ดสีขาวสะอาดลอยตัวด้วยเงานุ่ม, มุมโค้งมากขึ้นกว่าเดิม (คงจากรอบก่อนหน้า) ผสานกับสี accent ทอง-บรอนซ์/น้ำตาลเข้มที่อ้างอิงจากสีจริงของโลโก้ Projexa (เปลี่ยนรอบนี้) ยังคงพื้นที่ว่างมากและความเรียบง่ายของแนวเดิมไว้ แต่เปลี่ยนจากพื้นผิวแบน/border-based แบบ Muji เดิม เป็นพื้นผิวที่มีมิติเบาๆ ด้วย shadow
...
อ้างอิงจาก mood board ที่ผู้เกี่ยวข้องส่งภาพตัวอย่างมาให้ (ภาพหน้าจอ dashboard ของระบบอื่นที่ไม่เกี่ยวข้องกับ Projexa — ใช้เป็นแรงบันดาลใจด้านสไตล์การ์ด-shadow และพื้นหลังครีม-ขาวเท่านั้น ไม่ใช่การลอกฟีเจอร์หรือโครงสร้างหน้าจอ): การ์ดสีขาวสะอาดลอยเหนือพื้นหลังโทนครีมอุ่น ให้ความรู้สึกโปร่งเบาแต่ยังสงบ ส่วนสี accent รอบนี้เปลี่ยนมาอ้างอิงจากสีจริงของโลโก้ Projexa (`assets/Logo-Projexa.png`) แทน mood board เดิม เพื่อให้หน้าตาระบบและแบรนด์ไปในทิศทางเดียวกัน: สีทอง-บรอนซ์ (gold) จาก mark ตัว "P" ของโลโก้ เป็นสี accent หลักที่ให้ความรู้สึกน่าเชื่อถือและมีคุณค่า ไม่ฉูดฉาด, สีน้ำตาลเข้ม/ช็อกโกแลต (umber) จาก wordmark "Projexa" ใช้เป็นจุดเน้นเฉพาะจุด (สถานะยืนยัน/สำเร็จ) เท่านั้น

- สีอ้างอิง: ครีม-ขาว (พื้นหลัง/การ์ด), ทอง-บรอนซ์ (accent หลัก — จากสีจริงของโลโก้), น้ำตาลเข้ม/อัมเบอร์ (accent รอง — จากสีจริงของโลโก้) — ยังคง**ไม่ใช้สีดำสนิทหรือสีขาวสนิทกับข้อความ** เพื่อลดความแข็งกระด้าง (ตัวอักษรหลักยังเป็นสีน้ำตาลเข้มอมเทา ไม่ใช่ `#000000`) แม้พื้นหลังการ์ดรอบนี้จะเปลี่ยนเป็นขาวล้วนแล้วก็ตาม
...
| พื้นเข้ม (เช่น `ink-900`) | ใช้เวอร์ชัน flat สีทองเดียวของโลโก้ — ตอนนี้ใช้ token `gold-500` ของระบบแทนได้เลย เพราะสี accent หลักของระบบตรงกับสีทองจริงของโลโก้แล้ว (ไม่ต้องคง literal hex แยกต่างหากเหมือนตอนที่ accent เคยเป็นเขียวเสจ) ไม่ใช้เวอร์ชัน 3D เต็มเพราะเงา/ไฮไลต์จะกลืนกับพื้นเข้ม |
...
หลักการ: ใช้ **neutral (ครีม/ขาว) เป็น 90% ของพื้นที่หน้าจอ** ส่วนสี accent (`gold`, `umber`) ใช้เฉพาะจุดที่ต้องดึงความสนใจจริงๆ (primary action, สถานะสำคัญ) ห้ามใช้ accent กับพื้นที่กว้าง
...
อ้างอิงจากสีจริงของโลโก้ Projexa (`assets/Logo-Projexa.png`) — ทอง-บรอนซ์จาก mark ตัว "P" เป็น primary, น้ำตาลเข้ม/ช็อกโกแลตจาก wordmark เป็น secondary ทุกคู่ผ่านการเช็ค contrast กับพื้นขาว/ครีมแล้ว

| Token | Hex | การใช้งาน |
|---|---|---|
| `gold-100` | `#F5E9D3` | พื้นหลัง tint ของปุ่ม/แท็กที่ active หรือถูกเลือก |
| `gold-500` | `#8B5E22` | Primary action (ปุ่มหลัก, ลิงก์, focus ring) — สีเอกลักษณ์ของแบรนด์รอบนี้ อ้างอิงจากสีทอง-บรอนซ์ของโลโก้ |
| `gold-600` | `#734B1A` | สถานะ hover/active ของ `gold-500` |
| `umber-100` | `#EDE3D8` | พื้นหลัง tint ของสถานะ "ยืนยันแล้ว/สำเร็จ" |
| `umber-500` | `#4A3220` | Secondary accent, สถานะ Confirmed/Approved/Done, อ้างอิงจากสีน้ำตาลเข้มของ wordmark โลโก้ |
...
| In Review | `info` |
| Confirmed / Approved | `umber-500` บน `umber-100` |
| In Progress | `gold-500` บน `gold-100` |
...
|---|---|
| **Button** | Primary = พื้น `gold-500` ตัวอักษร `cream-050`; Secondary = พื้นโปร่ง border `cream-200` ตัวอักษร `ink-900`; Destructive = พื้นโปร่ง ตัวอักษร `danger` border `danger` เฉพาะ hover ค่อยเติมพื้น radius `8px` มีปุ่มระดับเดียวต่อหน้าจอที่เป็น Primary เท่านั้น (ห้ามมี Primary หลายปุ่มแข่งกัน) |
| **Input / Textarea / Select** | พื้น `cream-050` border `1px cream-200` radius `8px` focus = border `gold-500` + ring บาง 2px สี `gold-100` label อยู่เหนือ field เสมอ (ไม่ใช้ placeholder แทน label) |
...
| **Table** | Header ใช้ `h3` scale บนพื้น `white-000`, แถวสลับสี `cream-050`/`white-000` แบบจางมาก (ไม่ใช่ zebra จัด), แถวที่ hover ใช้ `gold-100` บางๆ, เส้นแบ่งแถวยังใช้ border `cream-200` ตามปกติ |
| **Tag / Badge** | ใช้กับ MoSCoW label, module code — พื้น bg-tint + ตัวอักษรสีเข้มของ token เดียวกัน (เช่น `umber-100`/`umber-500`) radius `8px` padding แน่น |
| **Status Pill** | ดู §2.1 ตารางสถานะ ต้องมีจุด (dot) นำหน้าข้อความเสมอ เพื่อแยกจาก Tag ทั่วไป |
| **Navigation (Sidebar)** | พื้น `white-000`, item ที่ active = พื้น `gold-100` + ตัวอักษร `gold-600` ไม่ใช้ไอคอนสีสันจัด ไอคอนเป็น outline บาง สีเดียวกับข้อความ |
| **Tabs** | เส้นใต้ (underline) บาง 2px สี `gold-500` สำหรับ tab ที่เลือก ไม่ใช้พื้นหลังเต็ม tab |
...
| **Pagination** | ตัวเลขเรียบ ไม่มีพื้นหลังเต็มที่หน้าปัจจุบัน ใช้แค่ตัวอักษร `gold-600` + ขีดล่างบาง |
...
| **Workflow Stepper** | แสดงขั้นตอน TOR → Requirement → Design → Test → Document ในภาพรวม | เส้นแนวนอน จุดที่ผ่านแล้ว = `umber-500`, จุดปัจจุบัน = `gold-500`, จุดที่ยังไม่ถึง = `cream-300` ห้ามใช้สีเดาสถานะที่ยังไม่เกิดขึ้นจริง |
...
- ทุก interactive element ต้องมี focus state ที่มองเห็นชัด (ring สี `gold-500`) รองรับการใช้ keyboard navigation ล้วน
